...
def valid_hook(hook: HookConfig) -> bool:
    ''' True if for every row/col there exists a subset whose gcd is the
    given one. '''
...
def solution(hook: HookConfig) -> Optional[Board]:
    ''' Given a hook configuration, output a solution to the puzzle, if
    a solution exists, otherwise output None (Using backtracking).'''
...
def blocked(cm: Choices) -> bool:
    ''' True iff there is a cell s.t. no 0 <= x < m is in the cell. '''
...
def nums(arr: list[int]) -> list[int]:
    ''' Similar to nums_subset, but with the possibility that values in
    arr are 'unknown'. '''
    ans = []
    read = True
    curr = 0
